...
import Link from "next/link";
import { BedDouble, Building2, CalendarClock, Home } from "lucide-react";
...
const services = [
  ...
  },
  {
    ...
    description:
      "Turnover cleaning for vacation rentals and Airbnb hosts in Calgary, including fresh linen staging, sanitizing and guest-ready resets.",
    icon: BedDouble,
    dark: false,
    href: "/airbnb-cleaning-services/",
    linkLabel: "View Airbnb Cleaning",
  },
  {
    title: "Seasonal Property Service",
    description:
    ...
    <section className="bg-white px-6 py-16 md:px-12 lg:px-24">
      <div className="container-custom mx-auto">
        <div className="mx-auto mb-12 max-w-3xl text-center">
          ...
          <p className="mt-5 leading-relaxed text-gray-600">
            Review our four core service categories and open the page that best
            matches your cleaning needs.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
          {services.map((service) => {
            ...
            return (
              <article
                key={service.title}
                className={`flex h-full flex-col rounded-3xl border p-8 shadow-sm transition-all duration-300 hover:shadow-md ${
                  service.dark
                    ? "border-[#0B4E9B] bg-[#0B4E9B] text-white"
                    : "border-[#0B4E9B]/20 bg-white text-[#0B4E9B]"
                }`}
