...
export default async function CustomersPage() {
  ...
  const [{ data: users }, { data: addresses }] = await Promise.all([
    supabase.from("users").select("id, customer_code, name, email, phone_number, source, created_at, is_blocked").eq("role", "customer").order("created_at", { ascending: false }),
  ...
  const customers: CustomerRecord[] = (users || []).map((user) => ({
    ...
    email: user.email || "",
    address: addressMap.get(user.id) || "",
  }));
